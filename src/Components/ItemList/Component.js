...
class ActivityList extends Component {
  ...
  render (){
    ...
    const activityListItem = activitiesList.length > 0 && activitiesList.map(activity => {
        const { id } = activity;
        return (
          <li key={id} id={`${id}`} className='itemlist' onClick={this.activityHandleClicked(id)} >
            <div className="callDate"><span>{moment(activity.created_at).format('MMMM, DD YYYY')}</span></div>
            <div className="callItem">
              <div className="iconCall">
                <img src={activity.call_type === 'answered' ? icoOutbound : icoInbound} />
              </div>
    ...
    return(
      <div className="activitiesBox">
          <ul className='list'>
              {activitiesList.length > 0 ? activityListItem : <li className="emptyList">No Activities !<br/>👍 All Archived !</li>}
          </ul>
      </div>
    )
  }
