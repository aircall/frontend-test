...
//LOADING
import {isLoading, isLoaded} from './../Commons/Loading/actions.js'
import {openDetails, closeDetails} from './../OpenDetails/actions.js'
import {currentNavIsInbox, currentNavIsAll} from './../Commons/Nav/actions.js'
...
export const archiveActivities = activities => async dispatch => {
    ...
    Promise.all (
        activities.map(async activity => {
            ...
            await axios.post(url, { "is_archived": true });
        }))
        .then(async (data)=>{
          dispatch(isLoaded())
...
export const resetActivities = () => async dispatch => {
    ...
    .catch((error) => dispatch(resetPayloadActivitiesFailure(error)) )
}
